        ...
        if filename.endswith(".zip"):
            ...
            for fileTxt in archive.filelist:
                if fileTxt.filename == "calendar.txt":
                    ...
                    timePrint = time.strftime(
                        "%a, %d %b %Y %H:%M:%S +0000", time.gmtime(os.path.getmtime(directoryGTFS + filename)))
                    print("{0} file\n calendar.txt -> start_date:{1}, end_date:{2} (first row)".format(
                        filename, res['start_date'][0], res['end_date'][0]))
...
def readConnections(gtfsDB, city, directoryGTFS, day, dayName):
    ...
    for trip in gtfsDB['trips'].find({'city': city}):
        try:
            tripID2Route[trip['file']][str(
                trip['trip_id'])] = str(trip['route_id'])
        except:
            tripID2Route[trip['file']] = {
                str(trip['trip_id']): str(trip['route_id'])}
        if count % 1000 == 0:
            # clear_output()
        ...
        if filename.endswith(".zip"):
            ...
            for nameCSVFile in listOfFile:
                ...
                if nameCSVFile in archive.namelist():
                    ...
                    res = []
                    fillConnections(
                        gtfsDB, stopsTimes, services_id[filename], city, filename, archive)
    indexConnections(gtfsDB)


def indexConnections(gtfsDB):
    gtfsDB['connections'].create_index(
        [("pStart", pym.ASCENDING), ("city", pym.ASCENDING)])
    gtfsDB['connections'].create_index(
        [("pEnd", pym.ASCENDING), ("city", pym.ASCENDING)])
    gtfsDB['connections'].create_index(
        [("tStart", pym.ASCENDING), ("tEnd", pym.ASCENDING), ("city", pym.ASCENDING)])
...
def checkNumberOfGtfs(gtfsDB, city):
    ...
    print('number of file in calendar+calendar_dates: {0}\nin stops: {1}'.format(
        len(namefiles), len(gtfsDB['stops'].distinct('file', filter={'city': city}))))
    return namefiles
...
def readValidityDateGtfs(gtfsDB, day, dayName, city):
    ...
    for name in namefiles:
        try:
            services_id[name]
            print('file: {0} \t total number of active service (in calendar.txt): {1}'.format(
                name, len(services_id[name])))
        except KeyError:
            print('file: {0} \t total number of active service (in calendar.txt): {1}'.format(
                name, 'Serv NOT FOUND!!'))
    ...
    for exp in gtfsDB['calendar_dates'].find({'date': day, 'city': city}):
        if (exp['exception_type'] == '1' or exp['exception_type'] == 1):
            ...
            if exp['service_id'] in services_id[exp['file']]:
                services_id[exp['file']].remove(exp['service_id'])
    tot = 0
    for name in namefiles:
        try:
            tot += len(services_id[name])
            print('file: {0} \t total number of active service (in calendar_dates.txt): {1}'.format(
                name, len(services_id[name])))
        except KeyError:
            print('file: {0} \t total number of active service (in calendar_dates.txt): {1}'.format(
                name, 'Serv NOT FOUND!!'))
    print('number of different service_id:', len(services_id),
          'total number of active services found:', tot)
    return services_id
...
def fillConnections(gtfsDB, stopsTimes, services_id, city, filename, archive):
    ...
    tot = gtfsDB['trips'].find(
        {'service_id': {'$in': services_id}, 'city': city, 'file': filename}).count()
    listTrip = list(gtfsDB['trips'].find(
        {'service_id': {'$in': services_id}, 'city': city, 'file': filename}))
    listToInsert = []
    listOfFreqTrip = {}
                    ...
                    if len(res) > 0:
                        while True:
                            for i, stop in enumerate(res[:-1]):
                                ...
                                diff = findSec(
                                    res[i+1]['arrival_time']) - findSec(res[i]['departure_time'])
                ...
                if len(res) > 1:
                    ...
                    if (startTime == infty or endTime == infty):
                        ...
                        for i, stop in enumerate(res[:-1]):
                            ...
                            res[i]['departure_time'] = res[i-1]['departure_time'] if findSec(
                                res[i]['departure_time']) == infty else res[i]['departure_time']
                            res[i]['arrival_time'] = res[i-1]['arrival_time'] if findSec(
                                res[i]['arrival_time']) == infty else res[i]['arrival_time']
                            tEnd = findSec(res[i]['arrival_time']) if findSec(
                                res[i+1]['arrival_time']) == infty else findSec(res[i+1]['arrival_time'])
...
def updateConnectionsStopName(gtfsDB, city):
    ...
    for stop in gtfsDB['stops'].find({'city': city}):
        res1 = gtfsDB['connections'].update_many({'city': city, 'pStart': stop['stop_id'], 'file': stop['file']}, {
                                                 "$set": {'pStart': stop['pos'], "updatedStart": True}})
        res2 = gtfsDB['connections'].update_many({'city': city, 'pEnd': stop['stop_id'], 'file': stop['file']}, {
                                                 "$set": {'pEnd': stop['pos'], "updatedEnd": True}})
        c1 += res1.modified_count
        c2 += res2.modified_count
        print('\r{0},{1}-- pStart {2} pEnd {3}, totC {4}'.format(count,
              tot, c1, c2, totC), end="\r")
        count += 1
    delStart = gtfsDB['connections'].delete_many(
        {'city': city, 'updatedStart': {"$exists": False}}).deleted_count
    delEnd = gtfsDB['connections'].delete_many(
        {'city': city, 'updatedEnd': {"$exists": False}}).deleted_count
...
def makeArrayConnectionsOld(gtfsDB, hStart, city):
    ...
    arrayCC = np.full((gtfsDB['connections'].find(
        {"city": city, 'tStart': {'$gte': hStart}}).count(), 4), 1., dtype=np.int)
    countC = 0
    tot = gtfsDB['connections'].find(
        {'tStart': {'$gte': hStart}, 'city': city}).count()
    for cc in allCC:
