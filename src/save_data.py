...
from read_conn import makeArrayConnections
import os
...
def listOfNeighbor(gtfsDB, city, limit_walking_time=900):
    ...
    print(
        f"len(P2SPos): {len(P2SPos)}, len(P2STime): {len(P2STime)}, len(P2PPos): {len(P2PPos)}, len(P2PTime): {len(P2PTime)}")
    return {
        'S2SPos': S2SPos,
        'S2STime': S2STime,
        'P2PPos': P2PPos,
...
def makeZipCitychrone(city, gtfsDB, arrayCC=[], path_main='./saved/', newScenario=False, budget=5000, costTubeKm=30, costMetroStop=100, metroLines=[], urlServerOsrm='localhost:5000', limit_walking_time=900):
    ...
    a = zipfile.ZipFile(path_main+city+'_citychrone.zip',
    ...
    dataCity = {}
    centerCityPoint = gtfsDB['points'].find_one(
        {'city': city}, sort=[('soc_score', -1)])
